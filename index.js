...
async function connectToMongo() {
	...
	try {
		...
		app.post("/buy/:name", sessionValidation, async (req, res) => {
			...
			const shopItems = await shopItemCollection
				.find({ name: itemName })
				.project({ price: 1 })
				.toArray();

			const userCollection = db.collection("users");
			const rerollAmount = await userCollection
				.find({ email: req.session.email })
				.project({ rerolls: 1 })
				.toArray();
		...
		app.get("/fitTasks", sessionValidation, async (req, res) => {
			...
			if (result[0].date != new Date().getDate()) {
				var taskBankFit = await db
					.collection("fitnessTasks")
					.find({})
					.project({ task: 1 })
					.toArray();
				...
				for (var i = 0; i < 3; i++) {
					while (true) {
						...
						if (
							taskBankFit[temp].task != userFitTasks[0] &&
							taskBankFit[temp].task != userFitTasks[1] &&
							taskBankFit[temp].task != userFitTasks[2]
						) {
				...
				for (var i = 0; i < 3; i++) {
					while (true) {
						...
						if (
							taskBankDiet[temp].task != userDietTasks[0] &&
							taskBankDiet[temp].task != userDietTasks[1] &&
							taskBankDiet[temp].task != userDietTasks[2]
						) {
				...
				res.render("fitTasks", {
					points: point,
					boostActive: timeRemaining.toFixed(2),
					task1: result[0].fitTasks[0],
					task2: result[0].fitTasks[1],
					task3: result[0].fitTasks[2],
					rerolls: result[0].rerolls,
					noRerolls: false,
				});
			}
		});
		...
		app.get("/dietTasks", sessionValidation, async (req, res) => {
			...
			if (result[0].date != new Date().getDate()) {
				...
				res.render("dietTasks", {
					points: point,
					boostActive: timeRemaining.toFixed(2),
					task1: result[0].dietTasks[0],
					task2: result[0].dietTasks[1],
					task3: result[0].dietTasks[2],
					rerolls: result[0].rerolls,
					noRerolls: false,
				});
			}
		...
		app.post("/signup", async (req, res) => {
			...
			if (error) {
				res.render("signup", {
					errorMessage: `Validation error: ${error.details[0].message}`,
					username,
					email,
				});
				return;
		...
		app.post("/reset-email", async (req, res) => {
			...
			try {
				...
				await db
					...
					.insertOne({ email, token: sessionToken, createdAt: new Date() });
    ...
    `,
		...
		app.post("/reset-password", async (req, res) => {
			...
			try {
				...
				await db
					.collection("users")
					.updateOne({ email: resetToken.email }, { $set: { password: hashedPassword } });
		...
		app.get("/rankProgress", sessionValidation, async (req, res) => {
			...
			const result = await usersCollection
				.find({ email: req.session.email })
				.project({ points: 1, rerolls: 1, user_rank: 1, _id: 1 })
				.toArray();
			...
			res.render("rankProgress", {
				points: result[0].points,
				rank: result[0].user_rank,
				nextRank: newRanking,
			});
		});
		...
		app.get("/main", async (req, res) => {
			...
			var results = await usersCollection
				.find({ email: req.session.email })
				.project({ fitTasks: 1, user_rank: 1, rerolls: 1, points: 1 })
				.toArray();
		...
		app.post("/rerollFit", sessionValidation, async (req, res) => {
			...
			if (result[0].rerolls < 1) {
				...
				res.render("fitTasks", {
					points: point,
					boostActive: ((req.session.hourTime - currentTime) / 60000).toFixed(2),
					task1: result[0].fitTasks[0],
					task2: result[0].fitTasks[1],
					task3: result[0].fitTasks[2],
					rerolls: result[0].rerolls,
			...
			while (true) {
				temp = Math.floor(Math.random() * taskBankFit.length);
				if (
					taskBankFit[temp].task != result[0].fitTasks[0] &&
					taskBankFit[temp].task != result[0].fitTasks[1] &&
					taskBankFit[temp].task != result[0].fitTasks[2]
				) {
					break;
				}
			}
		...
		app.post("/rerollDiet", sessionValidation, async (req, res) => {
			...
			while (true) {
				temp = Math.floor(Math.random() * taskBankFit.length);
				if (
					taskBankFit[temp].task != result[0].dietTasks[0] &&
					taskBankFit[temp].task != result[0].dietTasks[1] &&
					taskBankFit[temp].task != result[0].dietTasks[2]
				) {
					break;
				}
			}
		...
		app.post("/addPointDiet", sessionValidation, async (req, res) => {
			...
			while (true) {
				temp = Math.floor(Math.random() * taskBankDiet.length);
				if (
					taskBankDiet[temp].task != result[0].dietTasks[0] &&
					taskBankDiet[temp].task != result[0].dietTasks[1] &&
					taskBankDiet[temp].task != result[0].dietTasks[2]
				) {
					break;
				}
			}
			...
			return groq.chat.completions.create({
				messages: [
					...
					// how it should behave throughout the conversation.
					{
						role: "system",
						content:
							"you are a helpful assistant to a fitness user. Your answer should be concise and directly address the question. Please remove all " +
		...
		app.post("/transcribe", async (req, res) => {
			// This is the end point for posting to the Google Speech API
		...
		app.post("/audio", (req, res) => {
			// This is the end point for receiving audio and initiating transcription
...
async function resizeImage(fileBuffer, width, height) {
	try {
		...
		const resizedBuffer = await sharp(fileBuffer).resize(width, height).toBuffer();
