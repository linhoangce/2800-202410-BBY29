...
async function setup() {
	...
	video = createCapture(VIDEO, () => {
		console.log("Video is capture ready");
	});
	video.size(256, 144);
	video.hide();

	await init(); // wait for model to initialize
	getPoses(); // start loop only after model is definitely loaded
}

let isDetecting = false;

async function getPoses() {
	// chec if detector exists AND video is actually loaded
	if (detector && video.elt.readyState === 4 && !isDetecting) {
		isDetecting = true;

		// not use await on the whole function call here
		// to prevent blocking the rest of the script
		detector.estimatePoses(video.elt).then((results) => {
			poses = results;
			isDetecting = false; // ready for next detection
		});
	}
...
function draw() {
	...
	// getPoses(); // detection runs "in the background"
		...
		if (keypoints) {
			...
			for (let j = 0; j < keypoints.length; j++) {
				...
				if (keypoint.score > 0.35) {
					// Only draw if accuracy point is above 0.35
					fill(255, 255, 255);
					// stroke();
